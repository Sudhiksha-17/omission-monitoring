...
## Deviations and corrections (appended 2026-06-22, after test)

These corrections are appended, not edited into the text above. The original
predictions are left intact so the pre-registration remains an honest record of
what was committed, including its mistakes.

### 1. The capability direction was pre-registered inconsistently — trend retracted

H2 (above) predicts the structured advantage is *largest for the weakest monitor
and shrinks as capability rises*. C5 in the Phase 2 amendment predicts the
advantage *increases with capability*, "largest at GPT-4o," and calls a
*monotone increasing* advantage a confirmation of H2. These are opposite
directions. Because the capability direction was committed both ways, no
capability-trend result can be claimed as pre-registered confirmation. The
capability trend is therefore retracted entirely. The reported finding is that
the structured advantage tracks holistic-baseline degeneracy, not capability:
it is large only where the holistic baseline is degenerate (gpt-4o-mini) and
null on the capable models. The "capability window" phrasing in the locked dev
claim set above is superseded by this retraction.

### 2. Phase 2 model ladder substituted after the Groq deprecation

The Phase 2 amendment names the ladder as llama3:latest / gpt-4o-mini / gpt-4o.
The Groq llama endpoints were deprecated mid-project, so the open-model slots
were filled with gpt-oss-20b and gpt-oss-120b, and gpt-4o was retained only as a
ceiling anchor. The frozen result set is gpt-4o-mini, gpt-oss-20b, gpt-oss-120b
(+ gpt-4o anchor); llama3 is excluded. Predictions in C4 and C5 keyed
specifically to Llama3 8B are moot and are not evaluated.

### 3. Reconciliation

The full claim set as confirmed on the held-out test, with all CIs, is in
RESULTS.md. Where this pre-registration and RESULTS.md differ, RESULTS.md is the
record of what was actually found; this document is the record of what was
predicted.