...
- **Fail-open under source removal (headline).** Full to output_only degradation is sensitivity-dominant on 8 of 9 cells (sensitivity drops 0.42–0.82). The pre-registered specificity-dominant prediction is refuted. The lone exception is the gpt-4o anchor under holistic prompting, which fails toward false alarms instead.
- **Structured prompting protects every model at output_only**, on BA and specificity, all CI-clean.
- **Holistic goes below chance at output_only on three models** (kappa −0.27 to −0.42); structured stays positive. Direction claimed; mechanism (presence heuristic vs prior-defaulting vs misinterpretation) not adjudicable from one-word outputs.

## Scoped / secondary

- Structure rescues a degenerate baseline at full source (gpt-4o-mini only), +0.142 BA, CI [+0.073, +0.210]; null on both gpt-oss models. Not a general specificity improvement. The gpt-4o-mini degenerate holistic arm is a prompt-reliability artifact, not a capability signature.
...
Code, pre-registration, and bootstrap outputs: https://github.com/Sudhiksha-17/omission-monitoring
